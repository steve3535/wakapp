...
count_low_vuln+=severity_list.count('Low')+severity_list.count('None')
...
for key in dict_oval_positive:
    ...
    for cve in dict_oval_positive[key][2]:
        print("-->","%-16s" % cve,"/",dict_oval_positive[key][2][cve])
...
print("#!/bin/bash")
print("if [ ! -s resultat-$(hostname | cut -d. -f1).txt ];then")
print(' echo "Nothing to do" > ./yupdte.log')
print("exit 0")
print("else")
